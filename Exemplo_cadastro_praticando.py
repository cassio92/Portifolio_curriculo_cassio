...
class MainWindow(tkinter.Tk):
    ...
    def __init__(self):
        ...
        ttk.Entry(self.mainframe, width=20, textvariable=self.cartao).grid(row=4, column=2, columnspan=3,
                                                                           sticky=tkinter.W)
        ...
        ttk.Entry(self.mainframe, width=50, textvariable=self.endereco).grid(row=6, column=2, columnspan=9,
                                                                             sticky=tkinter.W)
        ...
        ttk.Entry(self.mainframe, width=20, textvariable=self.telefone).grid(row=9, column=2, columnspan=3,
                                                                             sticky=tkinter.W)

        # Celular
        ttk.Label(self.mainframe, text='Celular: ').grid(row=10, column=1, sticky=tkinter.E)
        self.celular = tkinter.StringVar()
        ttk.Entry(self.mainframe, width=20, textvariable=self.celular).grid(row=10, column=2, columnspan=3,
                                                                            sticky=tkinter.W)
    ...
    def novo(self, *ignore):
        reply = messagebox.askyesno('Novo',
                                    'Deseja salvar alterações para o paciente {0}?'.format(self.nome.get()),
                                    parent=self)
    ...
    def abrir(self, *ignore):
        ...
        if len(records) > 1:
            ...
            messagebox.showinfo(
                message='Há mais de um paciente com o nome {0} Registros: ({1}) Tente abrir pelo número do registro' \
    ...
    def abrir_id(self, *ignore):
        cursor = self.db.cursor()
        cursor.execute(
            "SELECT pacientes.nome, pacientes.sexo, pacientes.cartao, pacientes.dia_nasc, pacientes.mes_nasc, "
    ...
    def salvar(self, *ignore):
        ...
        if identity is None:
            ...
            cursor = self.db.cursor()
            cursor.execute(
                "UPDATE pacientes SET nome=:nome, sexo=:sexo, cartao=:cartao, dia_nasc=:dia_nasc, mes_nasc=:mes_nasc, "
    ...
    def callback(self, *ignore):
        try:
            ...
            self.n = datetime.date.today() - datetime.date(int(self.ano_nasc.get()), self.mes_idade,
                                                           int(self.dia_nasc.get()))
            self.idade.set(str(self.n.days / 365.25)[:5])
        except:
            self.idade.set('Invalid')

    def importar_db(self, *ignore):
        reply = messagebox.askyesno(title='Info',
                                    message='Esta ação irá apagar o banco de dados atual e importar um novo XML. Deseja continuar?')
        if not reply:
            return
        ...
        try:
            ...
            messagebox.showwarning(title='Erro',
                                   message='ERRO: {0}. Não foi possível importar o banco de dados'.format(err))
            return
        ...
        for element in tree.findall("pac"):
            ...
            messagebox.showinfo(title='Info',
                                message='Arquivo XML importado com sucesso. Foram importados {0} pacientes'.format(
                                    self.pac_count(self.db)))
    ...
    def exportar_db(self, *ignore):
        ...
        try:
            ...
            messagebox.showwarning(title='Erro',
                                   message='ERRO: {0}. Não foi possível exportar o banco de dados'.format(err))
